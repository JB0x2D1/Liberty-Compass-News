...
public class WidgetConfig extends Activity {
	...
	//DONE: select double click speed fast=200 medium=300 slow=400
	//DONE: only update the new widget, not all of them
	
	private int mAppWidgetId;
	private int textColor = Color.BLACK;
	private int widgetBg = R.drawable.bg_grey;
	private long updateFrequency = 14400000;
...
// spinner - choose widget background
		...
		spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
					int pos, long id) {
				int[] arr = {R.drawable.bg_black, R.drawable.bg_grey, R.drawable.bg_white, R.drawable.bg_clear};
...
// spinner - update news story (download from server) frequency
        Spinner spinnerUpdate = (Spinner) findViewById(R.id.spinner_update_frequency);
		ArrayAdapter<CharSequence> adapterUpdate = ArrayAdapter.createFromResource(this, R.array.update_intervals_array, android.R.layout.simple_spinner_item);
		...
		spinnerUpdate.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
					int pos, long id) {
				int[] arr = {86400000, 43200000, 21600000, 14400000, 7200000, 3600000};
				updateFrequency = (arr[pos]);
				if (pos > 3) Toast.makeText(view.getContext(), "Downloading more frequently causes battery drain.", Toast.LENGTH_LONG).show();
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});	
// spinner - set double click speed
		Spinner spinnerSpeed = (Spinner) findViewById(R.id.spinnerspeed);
		ArrayAdapter<CharSequence> adapterSpeed = ArrayAdapter.createFromResource(this, R.array.speeds_array, android.R.layout.simple_spinner_item);
		...
		spinnerSpeed.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
					int pos, long id) {
				int[] arr = {200, 300, 400};
				doubleClickDelay = (arr[pos]);
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});	
// text field - hex value of text color
        final EditText editText = (EditText) findViewById(R.id.widget_text_color);
        editText.setText("000000"); 
// text field - sample of how chosen text color will look
        final TextView textView = (TextView) findViewById(R.id.previewtext);
        textView.setTextColor(Color.BLACK); //default is black
// sliders - RGB set text color 
    ...
    private void sendFirstUpdate() { //call widget onUpdate
    	...
    	setColorIntent.setAction(MyWidgetProvider.ACTION_UPDATE_COLOR+String.valueOf(mAppWidgetId));
    	setColorIntent.putExtra("color", textColor);
    	setColorIntent.putExtra("widgetbg", widgetBg);
    	setColorIntent.putExtra("updatefrequency", updateFrequency);
    	setColorIntent.putExtra("doubleclickdelay", doubleClickDelay);
    	...
    	firstUpdate.setAction(MyWidgetProvider.ACTION_NEXT_HEADLINE+String.valueOf(mAppWidgetId));
