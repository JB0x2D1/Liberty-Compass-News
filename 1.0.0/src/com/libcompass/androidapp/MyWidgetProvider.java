...
public class MyWidgetProvider extends AppWidgetProvider {
	...
	//TODO: option to include archived links
	...
	private static int doubleClickDelay = 200;//time to wait for a second click in milliseconds
	...
	public void onReceive(final Context context, final Intent intent) {
    	...
    	if(AppWidgetManager.getInstance(context) != null) { appWidgetManager = AppWidgetManager.getInstance(context); }
    	else return;
...
// right side of widget was clicked
		// query server for new headlines on single click, reocnfigure widget on double click
		if (action.startsWith(ACTION_DOWNLOAD)) {
			widgetId = Integer.parseInt(action.substring(ACTION_DOWNLOAD.length()));
			//get current click count
	        int clickCount = context.getSharedPreferences("widget", Context.MODE_PRIVATE).getInt("rightclicks"+String.valueOf(widgetId), 0);
	        //increment and save current click count
	        context.getSharedPreferences("widget", Context.MODE_PRIVATE).edit().putInt("rightclicks"+String.valueOf(widgetId), ++clickCount).commit();
	        final Handler handler = new Handler() {
	            public void handleMessage(Message msg) {
	                int clickCount = context.getSharedPreferences("widget", Context.MODE_PRIVATE).getInt("rightclicks"+String.valueOf(widgetId), 0);
	                if (clickCount > 1) {
	         //Double Click - reconfigure widget
	        ...
	        //Single Click - query server for new headlines 
	        			...
	        			if (interval > 600000) {//600000 == 10 minutes
	        				if (!MainActivity.networkAvailable(context)) {
	        					Toast.makeText(context, "Can't reach LibCompass.com", Toast.LENGTH_LONG).show();
	        					return;
	        				}
	                ...
	                context.getSharedPreferences("widget", Context.MODE_PRIVATE).edit().putInt("rightclicks"+String.valueOf(widgetId), 0).commit();
	            }
	        };
	        ...
	        if (clickCount == 1) new Thread() {
	            ...
	            public void run(){
	                try {
	                    synchronized(this) { wait(doubleClickDelay); }
...
//save user-selected options from configuration activity
	...
	private void loadEverything(Context context, int widgetId) {
		...
		doubleClickDelay = context.getSharedPreferences("widget", Context.MODE_PRIVATE).getInt("doubleclickdelay", 200);
		updateFrequency = context.getSharedPreferences("widget", Context.MODE_PRIVATE).getLong("updateFrequency", 14400000);
		updated = context.getSharedPreferences("widget", Context.MODE_PRIVATE).getLong("updated", 0);
	...
	protected static void downloadAndSave(final Context context, final boolean toast, final OnTaskCompleted otc) {
		...
		final OnTaskCompleted downloadComplete = new OnTaskCompleted() {
			public void onTaskCompleted() {
				if(downloadSaveError == null && toast) {
					Toast.makeText(context, "Download Successful", Toast.LENGTH_LONG).show();
					updated = new Date().getTime();
				}
				else if (toast) Toast.makeText(context, "Download Error: " + downloadSaveError, Toast.LENGTH_LONG).show();
				Thread t = new Thread() {
		    		public void run() {
